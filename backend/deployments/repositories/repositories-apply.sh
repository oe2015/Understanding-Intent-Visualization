...
if [ -z "$1" ] ; then
    ...
    c_echo $YELLOW "Usage: $0 <environment> [-a|--auto]"
    exit 1
fi
...
pushd deployments/repositories > /dev/null
    ...
    # Terraform apply again to ensure all resources are created and configur all tainted resources
    terraform apply -var-file=../../environments/${ENVIRONMENT}.tfvars.json $INPUT_FALSE $AUTO_APPROVE
    if [ "$?" -ne "0" ]; then
        echo "Error in terraform apply!"
        exit 1
