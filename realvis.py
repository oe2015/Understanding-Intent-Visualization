...
page = st.sidebar.radio("Go to", ("Framings", "Persuasion Techniques"))
...
if page == "Framings":
    ...
    if 'selected_pairs' not in server_state:
        with server_state_lock["selected_pairs"]:
            server_state.selected_pairs = []
    ...
    selected_sources = [pair[1] for pair in server_state.selected_pairs if pair[0] == country]
    available_sources = [source for source in selected_sources_dict.keys() if source not in selected_sources]

    source = st.selectbox('Select source', available_sources, key='source')

    if st.button('Add selection'):
        with server_state_lock["selected_pairs"]:
            server_state.selected_pairs.append((country, source))

    if st.button('Remove last selection') and server_state.selected_pairs:
        with server_state_lock["selected_pairs"]:
            server_state.selected_pairs.pop()

    selected_countries = [pair[0] for pair in server_state.selected_pairs]
    selected_sources = [pair[1] for pair in server_state.selected_pairs]
    ...
    melted_df = filtered_df.reset_index().melt(id_vars=['source', 'total_articles'], var_name='Framing', value_name='Percentage')
    ...
    # Sort the DataFrame by 'source' and 'Percentage' in descending order
    ...
    st.plotly_chart(fig)
elif page == "Persuasion Techniques":
        # Number of countries
    ...
    available_countries = country_to_media_subtask3['country'].unique().tolist()

    country = st.selectbox('Select country', available_countries, key='country')
    source_counts_list = country_to_media_subtask3[country_to_media_subtask3['country'] == country]['source_frequencies'].values[0]
